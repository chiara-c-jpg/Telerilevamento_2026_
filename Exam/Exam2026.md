...
A differenza dei dati già elaborati, le immagini di Sumatra vengono prima processate per estrarre l'indice NDVI.
```r
# Calcolo Indici spettrali per Sumatra (Riau): DVI, NDVI, NDMI
# apertura pacchetti
library(terra)
library(viridis)

# Assicurati di aver caricato le immagini grezze
# sumatra2015_raw <- rast("Sumatra2015.tif") 
# sumatra2025_raw <- rast("Sumatra2025.tif")

# --- 1. DVI (Difference Vegetation Index) ---
# Formula: NIR - RED
dvi_sum2015 <- sumatra2015_raw[[4]] - sumatra2015_raw[[3]]
dvi_sum2025 <- sumatra2025_raw[[4]] - sumatra2025_raw[[3]]

par(mfrow=c(1, 2))
plot(dvi_sum2015, main="DVI Sumatra (2015)")
plot(dvi_sum2025, main="DVI Sumatra (2025)")

# Differenza DVI
dvi_diff_sum <- dvi_sum2025 - dvi_sum2015
cl_diff <- colorRampPalette(c("red","white","blue"))(100)
par(mfrow=c(1,1))
plot(dvi_diff_sum, col = cl_diff, main = "Differenza DVI Sumatra (2025 - 2015)")


# --- 2. NDVI (Normalized Difference Vegetation Index) ---
# Formula: (NIR - RED) / (NIR + RED)
ndvi_sum2015 <- (sumatra2015_raw[[4]] - sumatra2015_raw[[3]]) / (sumatra2015_raw[[4]] + sumatra2015_raw[[3]])
ndvi_sum2025 <- (sumatra2025_raw[[4]] - sumatra2025_raw[[3]]) / (sumatra2025_raw[[4]] + sumatra2025_raw[[3]])

par(mfrow=c(1, 2))
plot(ndvi_sum2015, col=viridis(100), main="NDVI Sumatra (2015)")
plot(ndvi_sum2025, col=viridis(100), main="NDVI Sumatra (2025)")

# Differenza NDVI
ndvi_diff_sum <- ndvi_sum2025 - ndvi_sum2015
par(mfrow=c(1,1))
plot(ndvi_diff_sum, col = cl_diff, main = "Differenza NDVI Sumatra (2025 - 2015)")


# --- 3. NDMI (Normalized Difference Moisture Index) ---
# Formula: (NIR - SWIR) / (NIR + SWIR) -> Assumendo SWIR alla banda 5
ndmi_sum2015 <- (sumatra2015_raw[[4]] - sumatra2015_raw[[5]]) / (sumatra2015_raw[[4]] + sumatra2015_raw[[5]])
ndmi_sum2025 <- (sumatra2025_raw[[4]] - sumatra2025_raw[[5]]) / (sumatra2025_raw[[4]] + sumatra2025_raw[[5]])

par(mfrow=c(1, 2))
plot(ndmi_sum2015, col=viridis(100), main="NDMI Sumatra (2015)")
plot(ndmi_sum2025, col=viridis(100), main="NDMI Sumatra (2025)")

# Differenza NDMI
ndmi_diff_sum <- ndmi_sum2025 - ndmi_sum2015
par(mfrow=c(1,1))
plot(ndmi_diff_sum, col = cl_diff, main = "Differenza NDMI Sumatra (2025 - 2015)")

# Caricamento dei raster grezzi
sumatra2015_raw <- rast("Sumatra2015.tif") 
sumatra2025_raw <- rast("Sumatra2025.tif")
...
soglie <- c(-1, 0.2, 1, 
             0.2, 0.6, 2, 
             0.6, 1, 3)
matrice_class <- matrix(soglie, ncol=3, byrow=TRUE)

# Classificazione
sumatra2015_class <- classify(ndvi_sum2015, matrice_class)
sumatra2025_class <- classify(ndvi_sum2025, matrice_class)

# Calcolo frequenze e percentuali
freq_sum2015 <- freq(sumatra2015_class)
freq_sum2025 <- freq(sumatra2025_class)

perc_sum2015 <- freq_sum2015$count * 100 / ncell(sumatra2015_class)
