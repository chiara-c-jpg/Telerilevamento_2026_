...
### Analisi di Confronto tra il raster del Borneo e quello di Sumatra
# 1. Caricamento delle immagini/raster di Sumatra salvati nella cartella
# (Se sono immagini raster GeoTIFF usa rast(), se sono immagini png/jpeg usa i comandi del tuo pacchetto imageRy)
sumatra2015 <- rast("Sumatra2015")
sumatra2025 <- rast("Sumatra2025")

# 2. Calcolo delle frequenze e percentuali per Sumatra
freq_sum2015 <- freq(sumatra2015)
freq_sum2025 <- freq(sumatra2025)

perc_sum2015 <- freq_sum2015$count * 100 / ncell(sumatra2015)
perc_sum2025 <- freq_sum2025$count * 100 / ncell(sumatra2025)

# 3. Creazione della tabella di Sumatra
...
# 4. Unione con la tabella del Borneo per il confronto finale
tabella_classi_borneo$Regione <- "Borneo (Kalimantan)"
tabella_classi_sumatra$Regione <- "Sumatra (Riau)"

dati_confronto <- rbind(tabella_classi_borneo, tabella_classi_sumatra)
dati_confronto$Classe <- factor(dati_confronto$Classe, 
                                levels = c("Suolo Nudo", "Piantagioni", "Foresta Primaria"))

# 5. Generazione dei grafici di confronto definitivi
...
# Ricreiamo correttamente la tabella del Borneo con i nomi giusti
tabella_classi_borneo <- data.frame(
  Classe = c("Suolo Nudo", "Piantagioni", "Foresta Primaria"),
  Anno_2015 = round(perc_2015, 2), # Oppure perc_borneo se le hai salvate così
  Anno_2025 = round(perc_2025, 2)
)

#  Creiamo la tabella di Sumatra
tabella_classi_sumatra <- data.frame(
  Classe = c("Suolo Nudo", "Piantagioni", "Foresta Primaria"),
  Anno_2015 = round(perc_sum2015, 2), # Modifica con il nome corretto della tua variabile 2015 di Sumatra se diverso
  Anno_2025 = round(perc_sum2025, 2)  # Modifica con il nome corretto della tua variabile 2025 di Sumatra se diverso
)

# Aggiungiamo l'etichetta della regione a entrambe
tabella_classi_borneo$Regione <- "Borneo (Kalimantan)"
tabella_classi_sumatra$Regione <- "Sumatra (Riau)"

# Uniamo le due tabelle in un unico dataframe di confronto
dati_confronto <- rbind(tabella_classi_borneo, tabella_classi_sumatra)

# Grafico di confronto per il 2015
p_comp_2015 <- ggplot(dati_confronto, aes(x = Classe, y = Anno_2015, fill = Regione)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  scale_fill_manual(values = c("Borneo (Kalimantan)" = "darkorange", "Sumatra (Riau)" = "purple")) +
  ylim(0, 100) +
...
# Grafico di confronto per il 2025
p_comp_2025 <- ggplot(dati_confronto, aes(x = Classe, y = Anno_2025, fill = Regione)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  scale_fill_manual(values = c("Borneo (Kalimantan)" = "darkorange", "Sumatra (Riau)" = "purple")) +
  ylim(0, 100) +
...
# 7. Unione dei due grafici con patchwork e stampa finale
grafico_confronto_finale <- p_comp_2015 + p_comp_2025
print(grafico_confronto_finale)
---
